...
@WebServlet(name = "TopKSchools", urlPatterns = {"/json/top-k-most-used-schools"})
public class TopKSchools extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (errorArray.size() == 0) {
                ...
                /*This method analyzes the data from database and calculates the total usageTime for all users
                 from every school before returning the ranks up till the k value.
                tkResult is sorted according to k 
                */
                ArrayList<String[]> tkResult = TopKController.topKSchools(startDate, endDate, appCategory, kValue);
                for (int i = 0; i < tkResult.size(); i++) {
                    String[] result = tkResult.get(i);
                    String[] schools = result[1].split(",");
                    for (int j = 0; j < schools.length; j++) {
                        // sort schools in ascending
                        Arrays.sort(schools);
                        JsonObject app = new JsonObject();
                        app.addProperty("rank", Integer.parseInt(result[0]));
                        app.addProperty("school", schools[j].toLowerCase());
